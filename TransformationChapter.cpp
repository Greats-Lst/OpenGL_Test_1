#include "TransformationChapter.h"
#include "Utility.h"
#include "stb_image.h"

int TransformationChapter::Exe()
{
	Utility::InitGLFW();
	...
	Shader shader("Shader/transformation.vs", "Shader/transformation.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		auto tranform_location = glGetUniformLocation(shader.ID, "transform");
		glUniformMatrix4fv(tranform_location, 1, GL_FALSE, glm::value_ptr(trans));
		...
		trans = glm::mat4(1.0);
		trans = glm::translate(trans, glm::vec3(-0.5f, 0.5f, 0.0f));
		trans = glm::scale(trans,glm ::vec3(sin(timeValue), sin(timeValue), 1.0f));
		glUniformMatrix4fv(tranform_location, 1, GL_FALSE, glm::value_ptr(trans));
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
